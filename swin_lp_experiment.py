    ...
    for lr in lrs:
        for seed in seeds:
            for l2pen in l2pens:
                ...
                if first_run:
                    df.to_csv('results/Swin_LP_hyperparameter_search.csv', mode="w", index=False, header=True) # overwrites if file already exists
                    first_run = False
                else:
                    df.to_csv('results/Swin_LP_hyperparameter_search.csv', mode="a", index=False, header=False)

...
def main():
    ...
    for lr in lrs:
        ...
        if first_run:
            df.to_csv('results/Swin_ST.csv', mode="w", index=False, header=True) # overwrites if file already exists
            first_run = False
        else:
            df.to_csv('results/Swin_ST.csv', mode="a", index=False, header=False)
        ...
        plt.legend()
        plt.savefig('results/Swin_ST_lr{}_seed{}.jpg'.format(str(lr)[2:], seed))
